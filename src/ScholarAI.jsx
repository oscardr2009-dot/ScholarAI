...
const QUICK_TOPICS = [
  {
    ...
    label: 'SAT Prep',
    prompt:
      "What's the best way to prepare for the SAT? Give me a full study plan.",
  },
  {
    icon: '🎓',
    label: 'ACT Tips',
    prompt:
      'How should I prepare for the ACT? What sections should I focus on?',
  },
  {
    icon: '📚',
    label: 'Study Methods',
    prompt:
      'What are the most effective study methods for high school students?',
  },
  {
    icon: '🏫',
    label: 'College Prep',
    prompt: 'What should I be doing in high school to prepare for college?',
  },
  {
    icon: '✏️',
    label: 'Essay Help',
    prompt: 'How do I write a strong college application essay?',
  },
  {
    icon: '🔬',
    label: 'AP Exams',
    prompt: 'How do I prepare for AP exams? Which ones are worth taking?',
  },
  {
    icon: '💼',
  ...
  },
  {
    icon: '🧮',
    label: 'Math Help',
    prompt: "I'm struggling with high school math. What resources can help me?",
  },
  {
    icon: '💻',
    label: 'Coding Intro',
    prompt:
      'I want to learn coding as a high schooler. Where should I start and what languages should I learn?',
  },
  {
    icon: '🧬',
    label: 'Science Help',
    prompt:
      'Help me understand key science concepts for high school biology, chemistry, and physics.',
  },
];
const CODING_LANGS = [
  'Python',
...
const SYSTEM_PROMPT = `You are ScholarAI, a friendly, knowledgeable, and highly encouraging AI assistant built to help high school students across the United States.
    ...
    <div className="att-row">
      {atts.map((a, i) => (
        <div key={i} className="att-item">
          ...
          {a.isVideo && a.thumb ? (
            <div className="att-vw">
              <img src={a.thumb} alt="" />
              <div className="play-b">▶</div>
            </div>
          ) : a.isVideo ? (
            <div className="att-icon vid">VID</div>
          ) : null}
  ...
  return (
    <div className={`mrow ${isU ? 'urow' : 'arow'}`}>
      ...
      <div className={`bub ${isU ? 'ubub' : 'abub'}`}>
        ...
        {msg.images?.length > 0 && (
          <div className="mimg-row">
            {msg.images.map((s, i) => (
              <img key={i} src={s} alt="" className="mimg" />
            ))}
          </div>
        )}
        {msg.videos?.length > 0 && (
    ...
    <div className="landing">
      ...
      <div className="land-card">
        <div className="land-logo">
          ...
          <span className="land-title">ScholarAI</span>
        </div>
        <div className="land-badge">High School AI · US Edition</div>
        {mode === 'home' && (
          <>
            <h1 className="land-h">Welcome to ScholarAI</h1>
...
// ─── ONBOARDING ───────────────────────────────────────────────────────────────
function Onboarding({ user, onStart }) {
    ...
    <div className="onboarding">
      ...
      <div className="ob-card">
        <div className="logo-lk">
          ...
          <span className="logo-tx">ScholarAI</span>
        </div>
    ...
    <div className="pj-panel">
      ...
      <div className="pj-list">
        ...
        {projects.map((p) => (
          <div
            key={p.id}
            className={`pj-item${p.id === selectedId ? ' pj-sel' : ''}`}
...
export default function ScholarAI() {
  ...
  const handleObStart = ({ grade: g, lang: l }) => {
    ...
    const welcome =
      g === 'prefer_not'
        ? "Hey there! 👋 I'm **ScholarAI** — your personal high school AI. You can chat, drop files, take photos, upload videos, write code, or manage projects. *I respond in any language!*"
        : `Hey! 👋 I'm **ScholarAI**, your guide for **${gl}**. Chat, upload files/photos/videos, get coding help, or start a project — I've got you! *I speak every language!*`;
    const id = uid();
    const chat = {
      id,
      title: 'Welcome',
          ...
          <div className={`sidebar${sideOpen ? '' : ' closed'}`}>
            <div className="sb-top">
              <div className="sb-logo">
                ...
                <span className="sb-logo-tx">ScholarAI</span>
              </div>
              <button
                className="sb-new"
                onClick={() => {
          ...
          <div className="main-area">
            ...
            <div className="ibar">
              <div className="ibar-inner">
                ...
                <p className="disc">
                  ScholarAI · AI-powered · All languages supported · Always
